...
|---|---|
| Backend | Django 5.2 |
| Database | SQLite (dev) / PostgreSQL (production, via `dj-database-url` + `psycopg2`) |
...
3. Add `MenuItem` entries under each category — mark items as `is_signature` to feature them on the homepage
4. Add `GalleryImage` entries with real photos
5. Reservations, reviews, contact messages, and orders placed on the site all appear in their respective admin sections

## Deployment

Deployed on [Render](https://render.com):

1. `gunicorn` and `whitenoise` handle the production server and static files
2. `DEBUG = False` with `ALLOWED_HOSTS` configured for the live domain
3. PostgreSQL database provisioned on Render, connected via `DATABASE_URL`
4. Media files (menu photos, gallery images) served through Cloudinary rather than local disk, since Render's filesystem isn't persistent
5. Environment variables (Razorpay keys, Cloudinary credentials, Twilio credentials, `SECRET_KEY`) set in the Render dashboard
6. Migrations run against the live database on deploy
